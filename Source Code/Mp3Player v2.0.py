...
class Mp3_player:
    ...
    def update_song(self):
        try:
            ...
            self.song_title.place(x=263, y=20, anchor="center")
    ...
    def check_endofsong(self):
        if self.terminate_thread or threading.active_count() >3:                                   # terminate threads that are not in use when you shuffle,play next/previous
            ...
            while True:
                ...
                current_time = int(self.player.time)
                if self.done:                                        # quits threads when quit condition is true
                    return None
                if n == 5:
                    self.garbage_collect()
                    n = 0
                if current_time == length or current_time == length + 1:
                    break
    ...
    def key_press(self,event):
        ...
        # if the playlist_loaded is false, throw error. If playlist_loaded is true, then give shortcut keys ability to function
            ...
            if not self.is_playlist_loaded:
                if self.key == 'w':
                    self.load_playlist()
                elif self.key == 'o'  or self.key =='e' or self.key == 's' or self.key == 'l' or self.key == 'i' or self.key == '.' or self.key == ',' or self.key == "p":
                    self.no_playlist()
                else:
                    pass
            elif self.is_playlist_loaded:
    ...
    def __init__(self,window , time_property):
        ...
        self.pause_button.place(x=70, y=80, anchor="center")
        self.resume_button.place(x=200, y=80, anchor="center")
        self.previous_button.place(x=330, y=80, anchor="center")
        self.next_button.place(x=455, y=80, anchor="center")
        self.load_song_button.place(x=250, y=140, anchor="center")
        self.load_playlist_button.place(x=120, y=110, anchor="center")
        self.shuffle_button.place(x=250, y=110, anchor="center")
        self.loop_button.place(x=380, y=110, anchor="center")
        self.song_time.place(x=260, y=50, anchor="center")

        self.volume_scale = tk.Scale(window, from_=0, to=100, orient=tk.HORIZONTAL, length=520, resolution=1,command= self.volume_adjust,bg= self.background_color,foreground="white",highlightthickness=0,troughcolor = "#303030",font = self.font,sliderlength=40)
        self.volume_scale.place(x=260, y=195, anchor="center")

        self.volume_text = tk.Label(text="Volume",bg= self.background_color,foreground="white",font = self.font)
        self.volume_text.place(x=0, y=170, anchor="w")
