...
class Mp3_player:
    ...
    def play(self):
        ...
        except ValueError:  # whenever the player has problems remake the audio player
            self.terminate_thread = True
            self.player.delete()
            self.player = pyglet.media.Player()
            self.volume_scale.set(10)
            self.player.volume = .10
            time.sleep(.05)  # make sures song plays properly without no hitches
            self.update_song()
    ...
    def check_endofsong(self):
        ...
        if self.terminate_thread or threading.active_count() > 3:  # terminate threads that are not in use when you shuffle,play next/previous etc
            self.terminate_thread = False
            return None
            ...
            while True:
                ...
                if length_song_hour == 0:
                    ...
                    self.song_time['text'] = "track  length    {}:{}/{}:{}".format(current_minute, current_sec,
                    length_song_minute,length_song_second)
                    ...
                    self.time_playback.set(self.conv_time)
                    self.song_time['text'] = "track  length    {}:{}:{}/{}:{}:{}".format(current_hour, current_minute,current_sec, length_song_hour,
                    length_song_minute,length_song_second)
    ...
    def __init__(self,window):
        ...
        pause_button = tk.Button(window, text="Pause", height=1, width=16, command=self.pause_song,borderwidth = 1,bg=button_color,foreground=self.text_color,font = self.font,relief="solid")
        resume_button = tk.Button(window, text="Resume", height=1, width=16, command=self.resume_song,borderwidth = 1,bg=button_color,foreground=self.text_color,font = self.font,relief="solid")
        shuffle_button = tk.Button(window, text="Shuffle", height=1, width=16, command=self.shuffle_music,borderwidth = 1,bg=button_color,foreground=self.text_color,font = self.font,relief="solid")
        next_button = tk.Button(window, text="Next", height=1, width=15, command=self.next_song,borderwidth = 1,bg=button_color,foreground=self.text_color,font = self.font,relief="solid")
        previous_button = tk.Button(window, text="Previous", height=1, width=16, command=self.previous_song,borderwidth = 1,bg=button_color,foreground=self.text_color,font = self.font,relief="solid")
        loop_button = tk.Button(window, text="Loop song", height=1, width=16, command=self.loop_song,borderwidth = 1,bg=button_color,foreground=self.text_color , font = self.font,relief="solid")
        ...
        volume_text = tk.Label(text="Volume", bg=self.background_color, foreground="white", font=self.font)
        ...
        self.status_bar = tk.Label(text="Status: None ", bg="#383838", foreground="white", font=self.font, width = 200)
        ...
        pause_button.place(x=78, y=95, anchor="center")
        resume_button.place(x=204, y=95, anchor="center")
        previous_button.place(x=330, y=95, anchor="center")
        next_button.place(x=452, y=95, anchor="center")

        shuffle_button.place(x=260, y=125, anchor="center")
        loop_button.place(x=385, y=125, anchor="center")
        ...
        volume_text.place(x=570, y=157, anchor="e")
